    ...
    for attempt in range(max_retries):
        try:
            ...
            if "429" in str(e) and attempt < max_retries - 1:
                ...
                continue
            return f"⚠️ Service Busy (429): Gemini's free tier is at capacity. Please wait 30s and try again."
...
with tab_tactical:
    ...
    leader_context = "Market Snapshot: "
    cols = st.columns(5)
    ...
    st.divider()
    query = st.text_input("Analyze Ticker (e.g. NVDA):").upper()
    if query:
        try:
...
with tab_research:
    ...
    with c1: st.write("### AI Analyst Desk")
    with c2: 
    ...
    if not api_key:
        ...
        # CLICKABLE SUGGESTIONS (Restored)
        ...
        final_query = prompt if prompt else clicked_suggestion
